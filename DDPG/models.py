"""
Model definitions for DDPG
Author: Nick LaFarge (reimplemented from OpenAI Spinning Up - DDPG)

See https://github.com/openai/spinningup/blob/master/spinup/algos/tf1/ddpg/core.py for original model implementation
"""
...
class Model(object):
    ...
    def trainable_vars(self):
        ...
        """
        return [x for x in tf.compat.v1.global_variables() if self.scope in x.name]


class Critic(Model):
    """
    Critic model for DDPG algorithm. Since multiple critics exist, this class provides a convenient way to produce
    ...
    def build_network(self, action_source, **kwargs):
        ...
        General network structure consistent with Spinning Up.  The specific network implementation is different,
        but the resulting network structure should be the same.
    ...
    def define_optimizers(self, r_ph, gamma, d_ph, q, q_pi_target):
        ...
        :param q_pi_target: target state-action value
        :return: loss as a function of the given placeholders and update_step
        """

        y_targ = tf.stop_gradient(r_ph + gamma * (1 - d_ph) * q_pi_target)

        # Standard MSE loss between computed q and the backup value
        loss = tf.reduce_mean(tf.compat.v1.squared_difference(q, y_targ))
...
class Actor(Model):
    """
    Actor model for DDPG algorithm. Since the actor and the target actor exists , this class provides a convenient way
